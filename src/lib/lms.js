...
/* SESSION */
    lms.session = lms.session || {};

    lms.session.userCookieName = 'Lms.LoginUser';
    
    lms.session.setUserCookie = function (user) {
        if (user) {
            lms.utils.setCookieValue(
                lms.session.userCookieName,
                JSON.stringify(user),
                new Date(new Date().getTime() + 1 * 86400000), //one day
                lms.appPath,
                lms.domain
            );
        } else {
            lms.utils.deleteCookie(lms.session.userCookieName, lms.appPath);
        }
    };

    lms.session.getUserCookie = function () {
       return lms.utils.getCookieValue(lms.session.userCookieName);
    };

    lms.session.removeUserCookie = function () {
       lms.utils.deleteCookie(lms.session.userCookieName, lms.appPath);
    };


/* AUTHORIZATION **********************************************/
    //Implements Authorization API that simplifies usage of authorization scripts generated by lms.

    lms.auth = lms.auth || {};

    lms.auth.tokenCookieName = 'Lms.AuthToken';

    lms.auth.setToken = function (authToken, expireDate) {
        lms.utils.setCookieValue(lms.auth.tokenCookieName, authToken, expireDate, lms.appPath, lms.domain);
    };

    lms.auth.getToken = function () {
        return lms.utils.getCookieValue(lms.auth.tokenCookieName);
    }

    lms.auth.clearToken = function () {
        lms.utils.deleteCookie(lms.auth.tokenCookieName);
    }
...
/**
     ...
     */
    lms.utils.setCookieValue = function (key, value, expireDate, path, domain) {
     ...
     */
    lms.utils.getCookieValue = function (key) {
     ...
     * @param {string} path (optional)
     */
    lms.utils.deleteCookie = function (key, path) {
        var cookieValue = encodeURIComponent(key) + '=';
